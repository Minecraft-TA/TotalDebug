...
## Array fix verified in the game

The user restarted Companion through F6. Process 50428 runs the content-hashed `3e9c68ca0a5cfa004f2f46c6015b428d55041399623f585f91f3bc1aa39c3c00` JAR and connects to Minecraft process 54328. [Live C13 replay](audit-evidence/2026-09-05/live-array-expression-fix.json) verifies all three evaluated array children advertise `[0]`, `[1]` and `[2]` expressions. Evaluating the copied second child expression succeeds with typed integer 2. The temporary breakpoint is removed, the worker finishes with 42, the debugger detaches and the temporary stdio process exits normally.

The local Codex MCP configuration now points to a copied, hash-verified `TotalDebugCompanion-672281c.jar`. A parsed before/after comparison verifies that only this server's classpath argument changed. Existing sidecars and this task's advertised schema were not restarted; the current stdio implementation was tested separately against the live application. A fresh MCP session is still needed to load the updated installed schema.

Overall stabilization is approximately 96%, an estimate rather than a completed release checklist. The next choice is the dedicated-server test environment. Operator/non-operator behavior, remaining debugger/soak coverage, the final scope decision on documented native/index limits, public dependency resolution, upstream CI and immutable candidate packaging still require closure before release. The evaluator remains in place. Unrelated project-planning edits remain uncommitted and outside this audit slice.
