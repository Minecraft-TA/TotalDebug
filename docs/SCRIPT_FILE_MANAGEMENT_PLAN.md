...
Duplicate uses the current open draft where one exists, not a stale disk snapshot. Do not overwrite the destination. Rename/move preserves a running script because execution already has an independent ID. Initially reject deletion of running scripts with an actionable Stop instruction; do not stop them implicitly while preparing a folder operation. Prepare every affected editor before changing any file.
...
Remap known saved breakpoint references and in-memory navigation history after a successful move. For deletion, report affected breakpoint references instead of leaving silent broken actions. Arbitrary Java strings and external paths are outside automatic reference rewriting.
...
Tests must cover duplicate basenames, nested script creation/open/save, invalid names and destination collisions, case-only Windows rename, root/traversal/junction boundaries, moving into a descendant, open dirty descendants, pending saves, replacement at an old path, duplicate-from-draft, running-script rename, rejected running deletion, partial failures, breakpoint references across runtime signatures, old analysis results, history navigation, folder expansion/selection and external nested changes. Retain the existing large-tree and no-unnecessary-rescan tests.
