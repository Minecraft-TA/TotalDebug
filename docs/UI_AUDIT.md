...
## Folder slice, 20 September 2026

Polish follow-up: file menus omit redundant Open source and folder-creation actions. Folder menus put creation first; both place Copy path above Delete. Rename/Delete show their shortcuts, and Move to handles multiple selections. Delete confirmation names the selected file or folder, defaults to Delete, and dismisses with Cancel or Escape. The shared tree context-menu hit test covers the full row width across Files, Find Usages, debugger values and script results.

Nested scripts now open as editable scripts and use full paths for tab identity. New Script and New Folder target the selected folder; rename, move, duplicate and deletion share editor-aware operations. Drag-and-drop is the primary move interaction. Moves preserve script editors and running state, remap saved breakpoint paths, and refresh affected folders. Failed writes cannot silently recreate old paths, and permanent deletion rejects folder aliases. The implementation retains automatic saving and the compact keyboard-driven name popup.
